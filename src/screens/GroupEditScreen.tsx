...
import { getGroupDetail, updateGroup, type GroupDetail } from '../services/groupApi';
import { launchImageLibrary } from 'react-native-image-picker';

export default function GroupEditScreen(): React.JSX.Element {
  const navigation = useNavigation<HomeNavigationProp>();
  const route = useRoute<RouteProp<HomeStackParamList, 'GroupEdit'>>();
  ...
  const fetchGroup = useCallback(async () => {
    try {
      ...
      setRules(data.rules.map(r => ({ title: r.title, body: r.body ?? '' })));
    } catch {
  ...
  const handleSave = async () => {
    if (!name.trim()) { showToast('Group name is required', 'warning'); return; }
    setSaving(true);
    try {
      await updateGroup(slug, {
        name: name.trim(),
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.header}>
          ...
          <Typography variant="body" style={styles.headerTitle}>Edit Group</Typography>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          <View style={styles.card}>
            <Typography variant="body" style={styles.sectionTitle}>Basic Info</Typography>
            <FormField label="Group Name *" value={name} onChangeText={setName} icon="account-group-outline" />
            <FormField label="Description" value={description} onChangeText={setDescription} icon="text-box-outline" multiline />
          </View>

          <View style={styles.card}>
            <Typography variant="body" style={styles.sectionTitle}>Visibility</Typography>
            <View style={styles.visibilityRow}>
              <TouchableOpacity style={[styles.visPill, visibility === 'public' && styles.visPillActive]} onPress={() => setVisibility('public')} activeOpacity={0.8}>
                <Icon name="earth" size={18} color={visibility === 'public' ? Colors.primary : Colors.gray500} />
                <Typography variant="caption" style={[styles.visPillText, visibility === 'public' && styles.visPillTextActive]}>Public</Typography>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.visPill, visibility === 'private' && styles.visPillActive]} onPress={() => setVisibility('private')} activeOpacity={0.8}>
                <Icon name="lock-outline" size={18} color={visibility === 'private' ? Colors.primary : Colors.gray500} />
                <Typography variant="caption" style={[styles.visPillText, visibility === 'private' && styles.visPillTextActive]}>Private</Typography>
              </TouchableOpacity>
            </View>
          </View>

          <View style={styles.card}>
            <Typography variant="body" style={styles.sectionTitle}>Images</Typography>
            <TouchableOpacity style={styles.imagePicker} onPress={() => pickImage('cover')} activeOpacity={0.8}>
              ...
              <Typography variant="caption" style={styles.imagePickerText}>{cover ? 'Cover selected' : 'Change Cover'}</Typography>
            </TouchableOpacity>
            <TouchableOpacity style={styles.imagePicker} onPress={() => pickImage('avatar')} activeOpacity={0.8}>
              <Icon name="image-outline" size={24} color={Colors.primary} />
              <Typography variant="caption" style={styles.imagePickerText}>{avatar ? 'Avatar selected' : 'Change Avatar'}</Typography>
            </TouchableOpacity>
          </View>

          <View style={styles.card}>
            <View style={styles.rulesHeader}>
              <Typography variant="body" style={styles.sectionTitle}>Rules</Typography>
            ...
            {rules.map((rule, idx) => (
              <View key={idx} style={styles.ruleInputWrap}>
                <TextInput style={styles.ruleInput} placeholder="Rule title" placeholderTextColor={Colors.gray400} value={rule.title} onChangeText={(text) => updateRule(idx, 'title', text)} />
                <TextInput style={[styles.ruleInput, { minHeight: 60 }]} placeholder="Description (optional)" placeholderTextColor={Colors.gray400} value={rule.body} onChangeText={(text) => updateRule(idx, 'body', text)} multiline />
  ...
  return (
    <View style={styles.fieldWrap}>
      ...
      <View style={styles.inputWrap}>
        ...
        <TextInput style={[styles.input, multiline && { minHeight: 80, textAlignVertical: 'top' }]} value={value} onChangeText={onChangeText} placeholderTextColor={Colors.gray400} multiline={multiline} />
      </View>
    </View>
  );
}
